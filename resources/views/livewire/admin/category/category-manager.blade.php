...
<div dir="rtl">
    ...
    <div class="card mt-6 p-4">
        <div>
            ...
            <input type="text" wire:model.live.debounce.300ms="search" id="search" placeholder="جستجو در نام یا والد..."
                ...
                @forelse ($categories as $category)
                    ...
                    <tr wire:key="category-{{ $category->id }}" class="table-row">
                        <td class="table-cell font-medium text-text-main">
                            <div class="flex items-center @if($category->children->isNotEmpty() && !$search) cursor-pointer @endif" @if($category->children->isNotEmpty() && !$search) wire:click="toggleExpand({{ $category->id }})" @endif>
                        ...
                        <td class="table-cell-muted">{{ $category->books_count }}</td>
                        <td class="table-cell-muted">
                            {{ \Morilog\Jalali\Jalalian::forge($category->created_at)->format('Y/m/d') }}
                        </td>
                        <td class="table-cell text-center">
                            <div class="flex items-center justify-center gap-x-4">
                                <button wire:click="edit({{ $category->id }})" class="btn-link" title="ویرایش">
                                    <x-icons.edit class="h-5 w-5"/>
                                </button>
                                <button wire:click="delete({{ $category->id }})"
                                        wire:confirm="آیا از حذف دائمی این دسته‌بندی اطمینان دارید؟"
                    ...
                    @if(in_array($category->id, $expanded) && !$search)
                        @foreach($category->children as $child)
                            <tr wire:key="child-{{ $child->id }}" class="table-row bg-surface-secondary/50">
                                ...
                                <td class="table-cell-muted">{{ $child->books_count }}</td>
                                <td class="table-cell-muted">
                                    {{ \Morilog\Jalali\Jalalian::forge($child->created_at)->format('Y/m/d') }}
                                </td>
                                <td class="table-cell text-center">
                                    <div class="flex items-center justify-center gap-x-4">
                                        <button wire:click="edit({{ $child->id }})" class="btn-link" title="ویرایش">
                                            <x-icons.edit class="h-5 w-5"/>
                                        </button>
