            ...
            <table class="table-main">
                ...
                @forelse ($composers as $composer)
                    <tr wire:key="composer-{{ $composer->id }}" class="table-row">
                        ...
                        <td class="table-cell-muted">{{ $composer->books_count }}</td>
                        <td class="table-cell-muted" title="{{$composer->description}}">{{ \Illuminate\Support\Str::limit($composer->description,60) }}</td>
                        <td class="table-cell-muted">
                            {{ \Morilog\Jalali\Jalalian::forge($composer->created_at)->format('Y/m/d') }}
                        </td>
                        <td class="table-cell text-center">
                            <div class="flex items-center justify-center gap-x-4">
                                <button wire:click="edit({{ $composer->id }})" class="btn-link" title="ویرایش">
                                    <x-icons.edit class="h-5 w-5"/>
                                </button>
                                <button wire:click="delete({{ $composer->id }})"
                                        wire:confirm="آیا از حذف دائمی این آهنگساز اطمینان دارید؟"
