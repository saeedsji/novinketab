...
<div dir="rtl">
    ...
    <div class="card mt-6 p-4">
        <div>
            ...
            <input type="text" wire:model.live.debounce.300ms="search" id="search" placeholder="جستجو در نام گوینده..." class="form-input mt-1">
        </div>
    </div>
            ...
            <table class="table-main">
                ...
                @forelse ($narrators as $narrator)
                    <tr wire:key="narrator-{{ $narrator->id }}" class="table-row">
                        ...
                        <td class="table-cell-muted text-center">{{ $narrator->books_count }}</td>
                        ...
                        <td class="table-cell text-center">
                            <div class="flex items-center justify-center gap-x-4">
                                <button wire:click="edit({{ $narrator->id }})" class="btn-link" title="ویرایش"><x-icons.edit class="h-5 w-5"/></button>
                                <button wire:click="delete({{ $narrator->id }})" wire:confirm="آیا از حذف دائمی این گوینده اطمینان دارید؟" class="btn-link-danger" title="حذف"><x-icons.trash-2 class="h-5 w-5"/></button>
                            </div>
                        </td>
                    </tr>
                @empty
