            ...
            <table class="table-main">
                ...
                @forelse ($authors as $author)
                    <tr wire:key="author-{{ $author->id }}" class="table-row">
                        ...
                        <td class="table-cell-muted">{{ $author->books_count }}</td>
                        <td class="table-cell-muted" title="{{$author->description}}">{{ \Illuminate\Support\Str::limit($author->description, 60) }}</td>
                        <td class="table-cell-muted">
                            {{ \Morilog\Jalali\Jalalian::forge($author->created_at)->format('Y/m/d') }}
                        </td>
                        <td class="table-cell text-center">
                            <div class="flex items-center justify-center gap-x-4">
                                <button wire:click="edit({{ $author->id }})" class="btn-link" title="ویرایش">
                                    <x-icons.edit class="h-5 w-5"/>
                                </button>
                                <button wire:click="delete({{ $author->id }})"
                                        wire:confirm="آیا از حذف دائمی این نویسنده اطمینان دارید؟"
