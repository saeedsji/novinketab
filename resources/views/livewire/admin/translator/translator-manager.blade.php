...
<div dir="rtl">
    ...
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-xl font-semibold text-text-main">مدیریت مترجمان</h1>
            <p class="mt-2 text-sm text-text-muted">ایجاد، ویرایش و مدیریت اطلاعات مترجمان.</p>
        </div>
        <div class="mt-4 sm:mt-0 sm:flex-none">
            <button wire:click="create" type="button" class="btn btn-primary">
                <x-icons.plus class="h-5 w-5"/>
                <span>افزودن مترجم</span>
            </button>
        </div>
    </div>
    ...
    <div class="card mt-6 p-4">
        <div>
            ...
            <input type="text" wire:model.live.debounce.300ms="search" id="search" placeholder="جستجو در نام مترجم..." class="form-input mt-1">
        </div>
    </div>
    ...
    <div class="mt-8 flex flex-col">
            ...
            <table class="table-main">
                ...
                <tr>
                    <th scope="col" class="table-header-cell">
                        <x-ui.sortable column="name" :$sortCol :$sortAsc>نام مترجم</x-ui.sortable>
                    </th>
                ...
                @forelse ($translators as $translator)
                    <tr wire:key="translator-{{ $translator->id }}" class="table-row">
                        <td class="table-cell font-medium text-text-main">{{ $translator->name }}</td>
                        <td class="table-cell-muted text-center">{{ $translator->books_count }}</td>
                        <td class="table-cell-muted hidden sm:table-cell">
                            {{ \Morilog\Jalali\Jalalian::forge($translator->created_at)->format('Y/m/d') }}
                        </td>
                        <td class="table-cell text-center">
                            <div class="flex items-center justify-center gap-x-4">
                                <button wire:click="edit({{ $translator->id }})" class="btn-link" title="ویرایش"><x-icons.edit class="h-5 w-5"/></button>
                                <button wire:click="delete({{ $translator->id }})" wire:confirm="آیا از حذف دائمی این مترجم اطمینان دارید؟" class="btn-link-danger" title="حذف"><x-icons.trash-2 class="h-5 w-5"/></button>
                            </div>
                        </td>
                    </tr>
                @empty
        ...
        @if($translators->hasPages())
            <div class="mt-4">{{ $translators->links() }}</div>
        @endif
    </div>


    {{-- =================================================================== --}}
    {{-- Translator Add/Edit Modal --}}
    {{-- =================================================================== --}}
            ...
            <form wire:submit="save">
                <div class="p-6">
                    ...
                    <div class="mt-6 space-y-6">
                        <div>
                            <label for="name" class="form-label">نام مترجم</label>
