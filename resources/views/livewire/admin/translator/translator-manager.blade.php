            ...
            <table class="table-main">
                ...
                @forelse ($translators as $translator)
                    <tr wire:key="translator-{{ $translator->id }}" class="table-row">
                        ...
                        <td class="table-cell-muted">{{ $translator->books_count }}</td>
                        <td class="table-cell-muted" title="{{$translator->description}}">{{ \Illuminate\Support\Str::limit($translator->description, 60) }}</td>
                        <td class="table-cell-muted">
                            {{ \Morilog\Jalali\Jalalian::forge($translator->created_at)->format('Y/m/d') }}
                        </td>
                        <td class="table-cell text-center">
                            <div class="flex items-center justify-center gap-x-4">
                                <button wire:click="edit({{ $translator->id }})" class="btn-link" title="ویرایش">
                                    <x-icons.edit class="h-5 w-5"/>
                                </button>
                                <button wire:click="delete({{ $translator->id }})"
                                        wire:confirm="آیا از حذف دائمی این مترجم اطمینان دارید؟"
