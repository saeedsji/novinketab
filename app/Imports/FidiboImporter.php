    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            try {
                ...
                $paymentType = $row[7]; // ستون هشتم: نوع فاکتور

                $saleAmount = (int)$saleAmount * 10;
                $publisherShare = (int)$publisherShare * 10;
                ...
                $platformId = $saleDateStr . '-' . $bookId . '-' . $saleAmount . '-' . $publisherShare . '-' . $paymentType;
                ...
                $paymentData = [
                    ...
                    'platform_share' => $saleAmount - $publisherShare,
                    'discount' => 0,
                    'tax' => 0,
                ];
                ...
                if ($payment->wasRecentlyCreated) {
                    ...
                    $this->failedDetails[] = ['row' => $row->toArray(), 'error' => "updateed book id in panel: {$bookId} , id : {$platformId}"];
                    $this->updatedRecords++;
                    continue;
                }
            } catch (\Exception $e) {
